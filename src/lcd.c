...
#define SOFT_TIMER_DELAY       32768
#define SOFT_TIMER_HANDLE      2
...
void displayInit()
{
    ...
    //           set up a 1 second repeating soft timer and uncomment the following lines
	  ...
	  timer_response = sl_bt_system_set_soft_timer(SOFT_TIMER_DELAY, SOFT_TIMER_HANDLE, SOFT_TIMER_SINGLE_SHOT);
	  if (timer_response != SL_STATUS_OK) {
	      LOG_ERROR("BT soft timer failed with an error response of %u\r\n", timer_response);
    }
